        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="row">
                <div class="col-xl">
                    <div class="card mb-4">
                        <div class="card-body">
                            <form class="jsFormValidate" action="{{ route('leaveapply.update', $model->id) }}" method="post">
                                @csrf 
                                @method('put')
                                <div class="row">
                                    <div class="col-md-3 mt-2">
                                        ...
                                        <select name="leave_id" class="form-select">
                                            ...
                                            @foreach($leavetype as $ltkey => $ltvalue)
                                                <option value="{{ $ltkey }}" @if($ltkey == $model->leave_id) selected @endif>{{ $ltvalue }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-3 mt-2">
                                        <label for="dateRangePicker" class="form-label">From Date <span class="text-danger">*</span></label>
                                        <input type="date" name="from_date" class="form-control" min="{{ $model->from_date }}" value="{{ $model->from_date }}"/>
                                    </div>
                                    <div class="col-md-3 mt-2">
                                        <label for="dateRangePicker" class="form-label">To Date <span class="text-danger">*</span></label>
                                        <input type="date" name="to_date" class="form-control" min="{{ $model->from_date }}" value="{{ $model->to_date }}"/>
                                    </div>
                                    <div class="col-md-3 mt-2">
                                        ...
                                        <select name="leave_mode" class="form-select">
                                            ...
                                            @foreach(config('constant.leave_mode') as $lmkey => $lmvalue)
                                                <option value="{{ $lmkey }}" @if($lmkey == $model->leave_mode) selected @endif>{{ $lmvalue }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-3 mt-2">
                                        ...
                                        <select name="is_leave_cancle" class="form-select">
                                            ...
                                            @foreach(config('constant.leave_cancle') as $lckey => $lcvalue)
                                                <option value="{{ $lckey }}" @if($lckey == $model->is_leave_cancle) selected @endif>{{ $lcvalue }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-12 mt-2">
                                        <label class="form-label" for="basic-default-fullname">Reason <span class="text-danger">*</span></label>
                                        <textarea name="reason" class="form-control">{{ $model->reason }}</textarea>
                                    </div>
                                </div>
                                <div class="mt-3">
